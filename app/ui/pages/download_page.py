...
class DownloadPage(QWidget):
    """Page for downloading market data."""
    ...
    def init_ui(self):
        ...
        self.tabs.addTab(self.terminal, "Terminal Output")

        self.status_widget = self._build_status_widget()
        self.tabs.addTab(self.status_widget, "Data Status")
    ...
    def _build_status_widget(self) -> QWidget:
        ...
        self.data_tree.setHeaderLabels(["Pair / File", "Timeframe", "Size"])
    ...
    def _refresh_data_status(self):
        """Scan user_data/data/binance/ and populate the data status tree."""
        ...
        for exchange_dir in sorted(data_dir.iterdir()):
            ...
            exchange_item.setForeground(0, QColor("#333"))
            ...
            for f in json_files:
                # filename format: PAIR-TIMEFRAME.json  e.g. BTC_USDT-5m.json
    ...
    def _load_preferences(self):
        ...
        if prefs.default_pairs:
            self.selected_pairs = [p.strip() for p in prefs.default_pairs.split(",") if p.strip()]
        else:
            self.selected_pairs = []
