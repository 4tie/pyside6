...
class LoopPage(QWidget):
    ...
    def _on_start(self) -> None:
        ...
        self._update_state_machine()
        self._set_status("Running backtest (iteration 1/" + str(config.max_iterations) + ")")

        # Kick off first iteration via QTimer to allow UI to update
    ...
    def _run_next_iteration(self) -> None:
        """Prepare and execute the next loop iteration."""
        ...
        if not self._loop_service._result.iterations:
            ...
            last = self._loop_service._result.iterations[-1]
            if last.summary is None:
                self._finalize_loop()
                return
            result = self._loop_service.prepare_next_iteration(last.summary)
        ...
        try:
            ...
            self._sandbox_dir = self._improve_service.prepare_sandbox(
                config.strategy, iteration.params_after
            )
            iteration.sandbox_path = self._sandbox_dir

            # Build and run backtest command
            ...
            self._export_dir = (
                ...
                / f"{config.strategy}_{ts}"
            )
            self._export_dir.mkdir(parents=True, exist_ok=True)

            extra_flags = [
                "--strategy-path", str(self._sandbox_dir),
                "--backtest-directory", str(self._export_dir),
            ]
            ...
            n = self._loop_service._current_iteration
            m = config.max_iterations
            self._set_status(f"Running backtest (iteration {n}/{m})")
            self._progress_bar.setValue(int(n / m * 100))
    ...
    def _on_backtest_finished(self, exit_code: int) -> None:
        """Called when the backtest subprocess finishes."""
        ...
        if exit_code != 0:
            ...
            self._gate_indicator.set_failed("in_sample", f"Exit code {exit_code}")
            self._add_history_row(iteration)
            QTimer.singleShot(100, self._run_next_iteration)
            return
        ...
        try:
            ...
            self._loop_service.record_iteration_error(iteration, f"Parse error: {exc}")
        ...
        # Record result (gates are run synchronously here for simplicity;
        # the UI updates gate indicator after each gate)
        self._loop_service.record_iteration_result(iteration, summary)

        # Update gate indicator from gate_results
        for gr in iteration.gate_results:
            if gr.passed:
                self._gate_indicator.set_passed(gr.gate_name)
            else:
                self._gate_indicator.set_failed(gr.gate_name, gr.failure_reason or "")

        self._add_history_row(iteration)
        self._update_stat_cards()

        if self._loop_result is None:
