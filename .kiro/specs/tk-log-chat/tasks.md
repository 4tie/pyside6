...
- [x] 1. Create script skeleton and environment loading
  - [x] 1.1 Create `sc/tk_log_chat.py` with module docstring, `load_env()`, module-level `OLLAMA_BASE_URL` / `OLLAMA_MODEL` constants with fallbacks, and `if __name__ == "__main__"` guard that calls `load_env()` and launches the app
  - [x] 1.2 Implement `load_env()`: call `dotenv.load_dotenv()` and read `OLLAMA_BASE_URL` (default `http://localhost:11434`) and `OLLAMA_MODEL` (default `llama3`) from environment variables into module-level constants

- [x] 2. Implement OllamaClient
  - [x] 2.1 Implement `OllamaClient.__init__`: initialise `self.history = []`
  - [x] 2.2 Implement `OllamaClient.health_check() -> tuple[bool, str]`: GET `{OLLAMA_BASE_URL}/api/tags`; return `(True, ready_message)` on HTTP 200, `(False, error_message_with_url)` on connection error or non-200 status; never raise
  - [x] 2.3 Implement `OllamaClient.chat(user_message: str) -> str`: append user message to `self.history`, build POST payload with system prompt + full history, POST to `{OLLAMA_BASE_URL}/api/chat` with `"stream": false`, extract `message.content` from response, append assistant reply to `self.history`, return reply; on any error (connection, non-200, malformed JSON) return a human-readable error string without raising

- [x] 3. Implement ChatApp UI
  - [x] 3.1 Implement `ChatApp.__init__(root, client)`: store references, call `_build_ui()`, then start health check on a background thread
  - [x] 3.2 Implement `ChatApp._build_ui()`: create Chat_Window (`tk.Text`, `state=DISABLED`, `wrap=WORD`) with vertical scrollbar filling the upper area; create Input_Area (`tk.Text`, `height=5`) and Send_Button (`ttk.Button`) in the lower area; set window title to "Log & Error Chat"; set `root.minsize(700, 500)`; bind Ctrl+Enter on Input_Area to `_on_send`; bind Send_Button command to `_on_send`
  - [x] 3.3 Implement `ChatApp._append_message(role, text)`: temporarily set Chat_Window to `NORMAL`, insert formatted text with the appropriate color tag (`user` → blue `#1a73e8`, `ai` → green `#2e7d32`, `system` → grey `#757575`), set back to `DISABLED`, call `_scroll_to_bottom()`
  - [x] 3.4 Implement `ChatApp._scroll_to_bottom()`: set Chat_Window yview to `tk.END`
  - [x] 3.5 Implement `ChatApp._health_check()`: run `client.health_check()` on a background thread; use `root.after(0, ...)` to call `_append_message` with the result on the main thread
