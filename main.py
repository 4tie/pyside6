...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Freqtrade GUI — graphical interface for the Freqtrade trading bot."
    )
    parser.add_argument(
        "--ui-v2",
        action="store_true",
        default=False,
        help="Launch the redesigned v2 UI (ModernMainWindow) instead of the default UI.",
    )
    # Pass only unknown args to QApplication so Qt's own flags (e.g. -platform)
    # are not consumed by argparse.
...
def main():
    ...
    if args.ui_v2:
        ...
        window = ModernMainWindow(settings_state=settings_state)
    else:
    ...
    log.info("Main window shown (ui_v2=%s)", args.ui_v2)
