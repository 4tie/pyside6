...
def run_backtest(strategy: str, timeframe: str, timerange: str = "") -> str:
    ...
    Args:
        strategy: Strategy name without .py (e.g. 'MyStrategy')
...
def read_backtest_results(strategy: str) -> dict:
    """Read the latest backtest result for a strategy (zip or JSON).
    ...
    if not results_dir.exists():
        return {"error": f"No results directory for {strategy}"}
...
def analyze_strategy_path(strategy: str) -> dict:
    """Return path, size, and first 30 lines of a strategy file.
...
def read_config() -> dict:
    ...
    return json.loads(cfg.read_text(encoding="utf8")) if cfg.exists() else {"error": "config.json not found"}


@mcp.tool()
def export_results_summary(strategy: str) -> str:
    """Return a plain-text metrics summary for the latest backtest of a strategy.
    ...
    try:
        ...
        return "\n".join([
            ...
            f"Wins     : {strat_data.get('wins', 'N/A')}",
